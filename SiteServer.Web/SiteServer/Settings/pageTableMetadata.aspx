<%@ Page Language="C#" Inherits="SiteServer.BackgroundPages.Settings.PageTableMetadata" %>
      ...
      <form class="form-inline" runat="server">
        ...
        <ul class="breadcrumb breadcrumb-button">
          ...
          <span style="<%=GetCreateDbCommandElementStyle()%>">
            <input type="button" class="btn" onclick="location.href='pageTableMetadata.aspx?CreateDB=True&ENName=<%=Request.QueryString["
              ENName "]%>    &TableType=<%=Request.QueryString["TableType "]%>    ';" value="创建辅助表" />
          </span>
          <span style="<%=GetDeleteDbCommandElementStyle()%>">
            <input type="button" class="btn" onclick="if (confirm('此操作将删除辅助表“<%=Request.QueryString[" ENName
              "]%>”，确认吗？'))location.href='pageTableMetadata.aspx?DeleteDB=True&ENName=<%=Request.QueryString["ENName "]%>    &TableType=<%=Request.QueryString["TableType "]%>    ';" value="删除辅助表" />
          </span>
          <span style="<%=GetReCreateDbCommandElementStyle()%>">
            <input type="button" class="btn" onclick="if (confirm('此操作将覆盖已建立的辅助表，表中已存数据将丢失，确认吗？'))location.href='pageTableMetadata.aspx?ReCreateDB=True&ENName=<%=Request.QueryString["
              ENName "]%>    &TableType=<%=Request.QueryString["TableType "]%>    ';" value="重新创建辅助表" />
          </span>
          <span style="<%=GetShowCommandElementStyle()%>">
            <input type="button" class="btn" onclick="location.href='pageTableMetadata.aspx?ShowCrateDBCommand=True&ENName=<%=Request.QueryString["
              ENName "]%>    &TableType=<%=Request.QueryString["TableType "]%>    ';" value="显示创建表SQL命令" />
          </span>
