    ...
    public partial class AuthenticatedRequest
    {
        ...
        public string ApiToken
        {
            get
            {
                ...
                if (StringUtils.EndsWith(accessTokenStr, Constants.EncryptStingIndicator))
                {
                    accessTokenStr = TranslateUtils.DecryptStringBySecretKey(accessTokenStr, WebConfigUtils.SecretKey);
                }
        ...
        {
            await DataProvider.SiteLogRepository.AddSiteLogAsync(siteId, channelId, 0, Administrator, action, summary);
        }

        public async Task AddSiteLogAsync(int siteId, int channelId, int contentId, string action, string summary)
        {
            await DataProvider.SiteLogRepository.AddSiteLogAsync(siteId, channelId, contentId, Administrator, action, summary);
        }

        public async Task AddAdminLogAsync(string action, string summary)
        {
            await DataProvider.LogRepository.AddAdminLogAsync(Administrator, action, summary);
        }

        public async Task AddAdminLogAsync(string action)
        {
            await DataProvider.LogRepository.AddAdminLogAsync(Administrator, action);
        }
        ...
        public async Task<string> AdminLoginAsync(string userName, bool isAutoLogin)
        {
            ...
            await DataProvider.LogRepository.AddAdminLogAsync(adminInfo, "管理员登录");
        ...
        public async Task<string> UserLoginAsync(string userName, bool isAutoLogin)
        {
            ...
            await DataProvider.UserLogRepository.AddUserLoginLogAsync(user.Id);
