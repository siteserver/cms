    ...
    public partial class PagesTemplateEditorController
    {
		...
		private async Task<GetResult> SaveAsync(Site site, Template request, AuthenticatedRequest auth)
		{
			...
			if (request.Id > 0)
			{
				...
				template = await TemplateManager.GetTemplateAsync(request.SiteId, templateId);
			...
			else
			{
				...
				template.Id = await DataProvider.TemplateRepository.InsertAsync(template, request.Content, auth.AdminName);
				await CreatePagesAsync(template);
				await auth.AddSiteLogAsync(request.SiteId,
        ...
        private Template GetTemplateResult(Template templateInfo, Site site)
        {
            ...
            if (templateInfo.Id > 0)
            {
                ...
                template.Content = TemplateManager.GetTemplateContent(site, templateInfo);
            }
            else
            {
