    ...
    public class MainController : ApiController
    {
        ...
        public IHttpActionResult GetConfig()
        {
            try
            {
                ...
                var pageUrl = rest.GetQueryString("pageUrl");
                ...
                if (!rest.IsAdminLoggin || rest.AdminInfo == null)
                {
                    ...
                    {
                        Value = false,
                        RedirectUrl = $"{AdminUtils.LoginUrl}?redirectUrl={PageUtils.UrlEncode(AdminUtils.GetIndexUrl(siteId, pageUrl))}"
                    });
                }
                if (rest.AdminInfo.Locked)
                {
                    ...
                    {
                        Value = false,
                        RedirectUrl = $"{AdminUtils.ErrorUrl}?message={PageUtils.UrlEncode("管理员账号已被锁定，请联系超级管理员协助解决")}"
                    });
                }
                ...
                if (siteInfo == null || !siteIdListWithPermissions.Contains(siteInfo.Id))
                {
                    if (siteIdListWithPermissions.Contains(rest.AdminInfo.SiteId))
                    {
                        ...
                        {
                            Value = false,
                            RedirectUrl = AdminUtils.GetIndexUrl(rest.AdminInfo.SiteId, pageUrl)
                        });
                    }
                    ...
                    if (siteIdListWithPermissions.Count > 0)
                    {
                        ...
                        {
                            Value = false,
                            RedirectUrl = AdminUtils.GetIndexUrl(siteIdListWithPermissions[0], pageUrl)
                        });
                    }
                    ...
                    {
                        Value = false,
                        RedirectUrl = $"{AdminUtils.ErrorUrl}?message={PageUtils.UrlEncode("您没有可以管理的站点，请联系超级管理员协助解决")}"
                    });
                }
                ...
                if (string.IsNullOrEmpty(defaultPageUrl))
                {
                    defaultPageUrl = AdminUtils.DashboardUrl;
                }
            ...
            if (siteInfo != null && siteIdListWithPermissions.Contains(siteInfo.Id))
            {
                ...
                else
                {
                    ...
                    foreach (var siteId in siteIdList)
                    {
                        ...
                        {
                            Href = AdminUtils.GetIndexUrl(site.Id, string.Empty),
                            Target = "_top",
                            Text = site.SiteName
                        });
                    }
