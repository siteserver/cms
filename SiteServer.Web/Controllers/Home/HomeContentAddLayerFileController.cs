...
{
    [OpenApiIgnore]
    [RoutePrefix("home/contentAddLayerFile")]
    public class HomeContentAddLayerFileController : ApiController
    {
        ...
        public IHttpActionResult Upload()
        {
            try
            {
                ...
                if (request.HttpRequest.Files.Count > 0)
                {
                    ...
                    {
                        return BadRequest("上传失败，上传附件格式不正确！");
                    }
                    if (!PathUtility.IsFileSizeAllowed(siteInfo, contentLength))
                    {
                        return BadRequest("上传失败，上传附件超出规定文件大小！");
                    }
