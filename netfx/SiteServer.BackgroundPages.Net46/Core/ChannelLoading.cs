            ...
            else if (loadingType == ELoadingType.Channel)
            {
                ...
                if (enabled)
                {
                    if (permissionsImpl.HasChannelPermissions(channelInfo.SiteId, channelInfo.Id, ConfigManager.ChannelPermissions.ChannelEdit))
                    {
                        ...
                        upLink =
                            $@"<a href=""{PageUtilsEx.GetCmsUrl(channelInfo.SiteId, nameof(PageChannel), new NameValueCollection
                            {
...
";
            ...
            else if (loadingType == ELoadingType.SiteAnalysis)
            {
                ...
                var num = channelInfo.ContentDao.GetCountOfContentAdd(siteInfo.Id, channelInfo.Id, EScopeType.All, startDate, endDate, string.Empty, ETriState.All);
                var contentAddNum = num == 0 ? "0" : $"<strong>{num}</strong>";

                num = channelInfo.ContentDao.GetCountOfContentUpdate(siteInfo.Id, channelInfo.Id, EScopeType.All, startDate, endDate, string.Empty);
