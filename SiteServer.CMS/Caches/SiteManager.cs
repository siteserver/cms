...
namespace SiteServer.CMS.Caches
{
    public static class SiteManager
    {
        private static class SiteManagerCache
        {
            ...
            public static List<KeyValuePair<int, SiteInfo>> GetSiteInfoKeyValuePairList()
            {
                ...
                lock (LockObject)
                {
                    ...
                    if (retval == null)
                    {
                        var list = DataProvider.Site.GetSiteInfoKeyValuePairList();
        ...
        //    }
        //    else if (StringUtils.EqualsIgnoreCase(tableName, DataProvider.Site.TableName))
        //    {
        //        tableStyle = ETableStyle.Site;
        //    }
        //    else if (StringUtils.EqualsIgnoreCase(tableName, DataProvider.Channel.TableName))
        //    {
        ...
        public static int GetParentSiteId(int siteId)
        {
            ...
            if (siteInfo != null && siteInfo.Root == false)
            {
                ...
                if (parentSiteId == 0)
                {
                    parentSiteId = DataProvider.Site.GetIdByIsRoot();
                }
            }
            return parentSiteId;
        }
        ...
        //                if (!siteIdList.Contains(siteId))
        //                {
        //                    var channelIdCollection = DataProvider.SitePermissions.GetAllPermissionList(permissionsImpl.Roles, siteId, true);
