...
namespace SiteServer.CMS.Caches.Content
{
    public static partial class ContentManager
    {
        ...
        public static IDictionary<string, object> Calculate(int sequence, ContentInfo contentInfo, List<ContentColumn> columns, Dictionary<string, Dictionary<string, Func<IContentContext, string>>> pluginColumns)
        {
            if (contentInfo == null) return null;

            var retVal = contentInfo.ToDictionary();
            ...
            foreach (var column in columns)
            {
                ...
                {
                    retVal[ContentAttribute.Sequence] = sequence;
                }
                else if (StringUtils.EqualsIgnoreCase(column.AttributeName, ContentAttribute.AdminId))
                {
                    ...
                    retVal[ContentAttribute.AdminId] = value;
                }
                else if (StringUtils.EqualsIgnoreCase(column.AttributeName, ContentAttribute.UserId))
                {
                    ...
                    retVal[ContentAttribute.UserId] = value;
                }
                else if (StringUtils.EqualsIgnoreCase(column.AttributeName, ContentAttribute.SourceId))
                {
                    retVal[ContentAttribute.SourceId] = SourceManager.GetSourceName(contentInfo.SourceId);
                }
                else if (StringUtils.EqualsIgnoreCase(column.AttributeName, ContentAttribute.AddUserName))
                {
                    ...
                    retVal[ContentAttribute.AddUserName] = value;
                }
                else if (StringUtils.EqualsIgnoreCase(column.AttributeName, ContentAttribute.LastEditUserName))
                {
                    ...
                    retVal[ContentAttribute.LastEditUserName] = value;
                }
            }
        ...
        public static bool IsCreatable(ChannelInfo channelInfo, ContentInfo contentInfo)
        {
            ...
            if (contentInfo.ReferenceId > 0 &&
                ETranslateContentTypeUtils.GetEnumType(contentInfo.Get<string>(ContentAttribute.TranslateContentType)) !=
