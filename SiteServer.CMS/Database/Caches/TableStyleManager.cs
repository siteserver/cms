...
namespace SiteServer.CMS.Database.Caches
{
    public static class TableStyleManager
    {
        ...
        public static string GetValidateInfo(TableStyleInfo styleInfo)
        {
            ...
            if (styleInfo.Extend.IsRequired)
            {
                builder.Append("必填项;");
            }
            if (styleInfo.Extend.MinNum > 0)
            {
                builder.Append($"最少{styleInfo.Extend.MinNum}个字符;");
            }
            if (styleInfo.Extend.MaxNum > 0)
            {
                builder.Append($"最多{styleInfo.Extend.MaxNum}个字符;");
            }
            if (styleInfo.Extend.ValidateType != ValidateType.None)
            {
                builder.Append($"验证:{ValidateTypeUtils.GetText(styleInfo.Extend.ValidateType)};");
            }
        ...
        private static TableStyleInfo GetDefaultContentTableStyleInfo(string tableName, string attributeName)
        {
            ...
            if (StringUtils.EqualsIgnoreCase(attributeName, nameof(ContentInfo.Title)))
            {
                ...
                styleInfo.Extend.VeeValidate = "required";
                styleInfo.Taxis = 1;
            }
        ...
        private static TableStyleInfo GetDefaultUserTableStyleInfo(string tableName, string attributeName)
        {
            ...
            else if (StringUtils.EqualsIgnoreCase(attributeName, nameof(UserInfo.Mobile)))
            {
                ...
                styleInfo.Extend.VeeValidate = "mobile";
                styleInfo.Taxis = 2;
            }
            else if (StringUtils.EqualsIgnoreCase(attributeName, nameof(UserInfo.Email)))
            {
                ...
                styleInfo.Extend.VeeValidate = "email";
                styleInfo.Taxis = 3;
            }
