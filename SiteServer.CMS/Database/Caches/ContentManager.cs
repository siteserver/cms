...
namespace SiteServer.CMS.Database.Caches
{
    public static class ContentManager
    {
        ...
        private static class ContentCache
        {
            ...
            public static ContentInfo GetContent(SiteInfo siteInfo, int channelId, int contentId)
            {
                lock (LockObject)
                {
                    ...
                    contentInfo = DataProvider.ContentRepository.GetCacheContentInfo(ChannelManager.GetTableName(siteInfo, channelId), channelId, contentId);
            ...
            public static ContentInfo GetContent(SiteInfo siteInfo, ChannelInfo channelInfo, int contentId)
            {
                lock (LockObject)
                {
                    ...
                    contentInfo = DataProvider.ContentRepository.GetCacheContentInfo(ChannelManager.GetTableName(siteInfo, channelInfo), channelInfo.Id, contentId);
                    dict[contentId] = contentInfo;
        ...
        public static void InsertCache(SiteInfo siteInfo, ChannelInfo channelInfo, IContentInfo contentInfo)
        {
            ...
            if (ETaxisTypeUtils.Equals(ETaxisType.OrderByTaxisDesc, channelInfo.Extend.DefaultTaxisType))
            {
                ...
                if (!isClearCache)
                {
                    var orderAttributeName =
                        ETaxisTypeUtils.GetContentOrderAttributeName(
                            ETaxisTypeUtils.GetEnumType(channelInfo.Extend.DefaultTaxisType));
        ...
        public static List<int> GetContentIdList(SiteInfo siteInfo, ChannelInfo channelInfo, int offset, int limit)
        {
            ...
            {
                var dict = ContentCache.GetContentDict(channelInfo.Id);
                var pageContentInfoList = DataProvider.ContentRepository.GetCacheContentInfoList(siteInfo, channelInfo, offset, limit);
            ...
            return DataProvider.ContentRepository.GetCacheContentIdList(siteInfo, channelInfo, offset, limit);
        }
        ...
        private static List<ContentCountInfo> GetContentCountInfoList(string tableName)
        {
            ...
            countList = DataProvider.ContentRepository.GetTableContentCounts(tableName);
        ...
        public static bool IsCreatable(ChannelInfo channelInfo, IContentInfo contentInfo)
        {
            ...
            return channelInfo.Extend.IsContentCreatable && string.IsNullOrEmpty(contentInfo.LinkUrl) && contentInfo.IsChecked && contentInfo.SourceId != SourceManager.Preview && contentInfo.ChannelId > 0;
        }
    }
}