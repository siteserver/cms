...
namespace SiteServer.CMS.Model.Db
{
	public class ChannelInfo : IChannelInfo
    {
        ...
        public async Task<Dictionary<string, object>> ToDictionaryAsync()
        {
            ...
            {
                jObject[nameof(ImageUrl)] = PageUtility.ParseNavigationUrl(site, ImageUrl, false);
            }

            jObject["NavigationUrl"] = await PageUtility.GetChannelUrlAsync(site, this, false);
            ...
            var list = Children.Select(async channelInfo => await channelInfo.ToDictionaryAsync()).ToList();
            dict["Children"] = list;
