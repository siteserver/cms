	...
	public class PermissionManager
	{
        ...
        public List<int> SiteIdList
        {
            get
            {
                ...
                _siteIdList = DataCacheManager.Get<List<int>>(_siteIdListKey);
                ...
                if (_siteIdList  == null)
                {
                    ...
                    DataCacheManager.InsertMinutes(_siteIdListKey, _siteIdList, 30);
                }
        ...
        public List<int> OwningChannelIdList
        {
            get
            {
                if (_owningChannelIdList == null)
                {
                    if (!string.IsNullOrEmpty(UserName) && !string.Equals(UserName, AdminManager.AnonymousUserName))
                    {
                        _owningChannelIdList = DataCacheManager.Get<List<int>>(_owningChannelIdListKey);
                        ...
                        if (_owningChannelIdList == null)
                        {
                            ...
                            DataCacheManager.InsertMinutes(_owningChannelIdListKey, _owningChannelIdList, 30);
                        }
                    }
                }
        ...
        {
            return DataCacheManager.GetCacheKey(nameof(PermissionManager), nameof(GetRolesCacheKey), userName);
        }

        private static string GetPermissionListCacheKey(string userName)
        {
            return DataCacheManager.GetCacheKey(nameof(PermissionManager), nameof(GetPermissionListCacheKey), userName);
        }

        private static string GetWebsitePermissionDictCacheKey(string userName)
        {
            return DataCacheManager.GetCacheKey(nameof(PermissionManager), nameof(GetWebsitePermissionDictCacheKey), userName);
        }

        private static string GetChannelPermissionDictCacheKey(string userName)
        {
            return DataCacheManager.GetCacheKey(nameof(PermissionManager), nameof(GetChannelPermissionDictCacheKey), userName);
        }

        private static string GetChannelPermissionListIgnoreChannelIdCacheKey(string userName)
        {
            return DataCacheManager.GetCacheKey(nameof(PermissionManager), nameof(GetChannelPermissionListIgnoreChannelIdCacheKey), userName);
        }

        private static string GetSiteIdListCacheKey(string userName)
        {
            return DataCacheManager.GetCacheKey(nameof(PermissionManager), nameof(GetSiteIdListCacheKey), userName);
        }

        private static string GetOwningChannelIdListCacheKey(string userName)
        {
            return DataCacheManager.GetCacheKey(nameof(PermissionManager), nameof(GetOwningChannelIdListCacheKey), userName);
        }

        public static void ClearAllCache()
        {
            DataCacheManager.RemoveByClassName(nameof(PermissionManager));
        }
    }
}
