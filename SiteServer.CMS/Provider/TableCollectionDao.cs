...
namespace SiteServer.CMS.Provider
{
    public class TableCollectionDao : DataProviderBase
	{
			...
			using (var conn = GetConnection()) 
			{
				...
				using (var trans = conn.BeginTransaction()) 
				{
					try 
					{
                        ...
                        //DataProvider.TableMetadataDao.InsertSystemItems(collectionInfo.TableEnName, trans);
		...
		{
            var fieldNum = DataProvider.TableMetadataDao.GetTableMetadataCountByEnName(tableEnName);
		...
		{
            var isDbExists = DataProvider.DatabaseDao.IsTableExists(tableEnName);
			...
			using (var conn = GetConnection()) 
			{
				...
				using (var trans = conn.BeginTransaction()) 
				{
					try 
					{
                        ...
                        DataProvider.TableMetadataDao.Delete(tableEnName, trans);
						trans.Commit();
					}
					catch
					{
        ...
        {
            if (!DataProvider.DatabaseDao.IsTableExists(tableEnName)) return;
        ...
        {
            var isTableExists = DataProvider.DatabaseDao.IsTableExists(tableEnName);
            ...
            using (var conn = GetConnection())
            {
                ...
                using (var trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        var createTableSqlString = DataProvider.DatabaseDao.GetCreateTableCollectionInfoSqlString(tableEnName);
        ...
        public void SyncDbTable(string tableEnName)
        {
            ...
            foreach (var metadataInfo in metadataInfoList)
            {
                ...
                foreach (var columnInfo in columnInfolist)
                {
                    ...
                    {
                        var dropColumnsSqlList = DataProvider.DatabaseDao.GetDropColumnsSqlString(tableEnName, metadataInfo.AttributeName);
                        foreach (var sql in dropColumnsSqlList)
                        {
                            sqlList.Add(sql);
                        }
                        var addColumnsSqlList1 = DataProvider.TableMetadataDao.GetAddColumnsSqlString(tableEnName, metadataInfo);
                ...
                var addColumnsSqlList2 = DataProvider.TableMetadataDao.GetAddColumnsSqlString(tableEnName, metadataInfo);
            ...
            foreach (var columnInfo in columnInfolist)
            {
                ...
                if (isNeedDelete)
                {
                    var dropColumnsSqlList = DataProvider.DatabaseDao.GetDropColumnsSqlString(tableEnName, columnInfo.ColumnName);
                    foreach (var sql in dropColumnsSqlList)
                    {
                        sqlList.Add(sql);
                    }
            ...
            DataProvider.TableCollectionDao.UpdateIsChangedAfterCreatedInDbToFalse(tableEnName);
            TableColumnManager.ClearCache();
        }
