...
namespace SiteServer.BackgroundPages.Cms
{
	public class PageConfigurationContent : BasePageCms
    {
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            SystemWebUtils.SelectSingleItemIgnoreCase(DdlIsSaveImageInTextEditor, SiteInfo.IsSaveImageInTextEditor.ToString());

            FxUtils.AddListItems(DdlIsAutoPageInTextEditor, "自动分页", "手动分页");
            SystemWebUtils.SelectSingleItemIgnoreCase(DdlIsAutoPageInTextEditor, SiteInfo.IsAutoPageInTextEditor.ToString());
            ...
            SystemWebUtils.SelectSingleItemIgnoreCase(DdlIsContentTitleBreakLine, SiteInfo.IsContentTitleBreakLine.ToString());

            //保存时，敏感词自动检测
            FxUtils.AddListItems(DdlIsAutoCheckKeywords, "启用敏感词自动检测", "不启用");
            SystemWebUtils.SelectSingleItemIgnoreCase(DdlIsAutoCheckKeywords, SiteInfo.IsAutoCheckKeywords.ToString());
            ...
            SystemWebUtils.SelectSingleItem(DdlIsCheckContentUseLevel, SiteInfo.IsCheckContentLevel.ToString());
            if (SiteInfo.IsCheckContentLevel)
            {
                SystemWebUtils.SelectSingleItem(DdlCheckContentLevel, SiteInfo.CheckContentLevel.ToString());
