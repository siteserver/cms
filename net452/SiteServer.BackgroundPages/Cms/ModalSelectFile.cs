	...
	public class ModalSelectFile : BasePageCms
    {
		...
		private string GetRedirectUrl(string path)
		{
            ...
            if (!DirectoryUtils.IsInDirectory(TopPath, path))
                path = TopPath;
            return FxUtils.GetCmsUrl(SiteId, nameof(ModalSelectFile), new NameValueCollection
            {
                {"RootPath", _rootPath},
        ...
        public static string GetOpenWindowString(int siteId, string hiddenClientId, string currentRootPath)
        {
            return LayerUtils.GetOpenScript("选择文件",
                FxUtils.GetCmsUrl(siteId, nameof(ModalSelectFile), new NameValueCollection
                {
                    {"RootPath", "@"},
		...
		public void Page_Load(object sender, EventArgs e)
        {
            ...
            WebPageUtils.CheckRequestParameter("siteId", "RootPath", "CurrentRootPath", "HiddenClientID");
			...
			{
                WebPageUtils.RedirectToErrorPage("文件夹不存在！");
                return;
			}
            ...
            {
                SystemWebUtils.SelectSingleItem(DdlListType, AuthRequest.GetQueryString("ListType"));
            }
		...
		public void LinkButton_Command(object sender, CommandEventArgs e)
		{
			...
			WebPageUtils.Redirect(navigationUrl);
		}
