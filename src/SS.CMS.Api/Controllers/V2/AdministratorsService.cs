...
namespace SS.CMS.Core.Services.V2
{
    public class AdministratorsService : ServiceBase
    {
        ...
        public AdministratorsService(IAccessTokenRepository accessTokenRepository)
        {
        ...
        {
            var isApiAuthorized = await _accessTokenRepository.IsScopeAsync(request.ApiToken, AccessTokenManager.ScopeAdministrators);
        ...
        public async Task<ResponseResult<object>> Update(IRequest request, IResponse response)
        {
        ...
        public async Task<ResponseResult<object>> Delete(IRequest request, IResponse response)
        {
            var id = TranslateUtils.Get<int>(request.RouteValues, "id");
            var isApiAuthorized = await _accessTokenRepository.IsScopeAsync(request.ApiToken, AccessTokenManager.ScopeAdministrators);
            if (!isApiAuthorized) return Unauthorized();
        ...
        public async Task<ResponseResult<object>> List(IRequest request, IResponse response)
        {
            var isApiAuthorized = await _accessTokenRepository.IsScopeAsync(request.ApiToken, AccessTokenManager.ScopeAdministrators);
            if (!isApiAuthorized) return Unauthorized();
        ...
        public async Task<ResponseResult<object>> ResetPassword(IRequest request, IResponse response)
        {
            var isApiAuthorized = await _accessTokenRepository.IsScopeAsync(request.ApiToken, AccessTokenManager.ScopeAdministrators);
            if (!isApiAuthorized) return Unauthorized();
