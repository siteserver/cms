...
namespace SSCMS.Core.Services.AuthManager
{
    public partial class AuthManager
    {
        ...
        private async Task<IList<string>> GetRolesAsync()
        {
            ...
            if (_administrator == null || _administrator.Locked)
                return new List<string> { PredefinedRole.Administrator.GetValue() };

            _roles = _cacheManager.Get<List<string>>(_rolesKey);
            if (_roles == null)
            {
                _roles = await _databaseManager.AdministratorsInRolesRepository.GetRolesForUserAsync(_administrator.UserName);
        ...
        private async Task<Dictionary<int, List<string>>> GetWebsitePermissionDictAsync()
        {
            ...
            if (_administrator == null || _administrator.Locked) return new Dictionary<int, List<string>>();
        ...
        private async Task<Dictionary<string, List<string>>> GetChannelPermissionDictAsync()
        {
            ...
            if (_administrator == null || _administrator.Locked) return new Dictionary<string, List<string>>();
