...
namespace SS.CMS.Plugins
{
    ...
    public static partial class PluginManager
    {
        ...
        private static async Task<SortedList<string, PluginInstance>> LoadAsync()
        {
            ...
            {
                await GlobalSettings.ErrorLogRepository.AddErrorLogAsync(ex, "载入插件时报错");
            }
        ...
        private static async Task<PluginInstance> ActivePluginAsync(string directoryName)
        {
            ...
            catch (Exception ex)
            {
                ...
                await GlobalSettings.ErrorLogRepository.AddErrorLogAsync(ex, $"插件加载：{directoryName}");
            }
        ...
        private static void CopyDllsToBin(string pluginId, string pluginDllDirectoryPath)
        {
            foreach (var filePath in DirectoryUtils.GetFilePaths(pluginDllDirectoryPath))
            {
                ...
                var fileName = PathUtils.GetFileName(filePath);
                var binFilePath = PathUtils.Combine(GlobalSettings.ContentRootPath, "Bin", fileName);
        ...
        private static async Task<SortedList<string, PluginInstance>> GetPluginSortedListAsync()
        {
        ...
        public static async Task UpdateDisabledAsync(string pluginId, bool isDisabled)
        {
            ...
            if (pluginInfo != null)
            {
                ...
                await GlobalSettings.PluginRepository.UpdateIsDisabledAsync(pluginId, isDisabled);
                ClearCache();
            }
        }
        ...
        public static async Task UpdateTaxisAsync(string pluginId, int taxis)
        {
            ...
            if (pluginInfo != null)
            {
                ...
                await GlobalSettings.PluginRepository.UpdateTaxisAsync(pluginId, taxis);
                ClearCache();
            }
        }
