    ...
    public class PagesAdministratorsController : ApiController
    {
        ...
        public IHttpActionResult GetList()
        {
            try
            {
                ...
                {
                    query.Where(nameof(AdministratorInfo.CreatorUserName), request.AdminName);
                }
                ...
                {
                    var userNameList = DataProvider.AdministratorsInRolesDao.GetUserNameListByRoleName(roleName);

                    query.WhereIn(nameof(AdministratorInfo.UserName), userNameList);
                }

                var count = DataProvider.AdministratorDao.GetCount(query);
                ...
                if (count > 0)
                {
                    ...
                    var tableColumns = DataProvider.AdministratorDao.TableColumns;
                    ...
                    administratorInfoList = DataProvider.AdministratorDao.GetAll(query).ToList();
                }

                var roles = RoleManager.GetRestRoles(request.AdminPermissions.IsSuperAdmin(), request.AdminName);
                ...
                foreach (var administratorInfo in administratorInfoList)
                {
                    ...
                    dynamicObj.NotMe = administratorInfo.Id != request.AdminId;

                    pageContents.Add(((Entity)dynamicObj).ToDictionary());
                }
        ...
        public IHttpActionResult Delete()
        {
            try
            {
                ...
                foreach (var adminId in adminIdList)
                {
                    ...
                    LogUtils.AddAdminLog(request.AdminName, "删除管理员", $"管理员:{adminInfo.UserName}");
                }
