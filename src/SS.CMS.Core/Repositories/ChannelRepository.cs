...
namespace SS.CMS.Core.Repositories
{
    public class ChannelRepository : IChannelRepository
    {
        ...
        public ChannelRepository(ISettingsManager settingsManager, IAdministratorRepository administratorRepository, IChannelGroupRepository channelGroupRepository, ISiteRepository siteRepository, ITemplateRepository templateRepository)
        {
        ...
        public int InsertSiteInfo(ChannelInfo channelInfo, SiteInfo siteInfo, string administratorName)
        {
            ...
            DataProvider.SiteRepository.Insert(siteInfo);

            var adminInfo = _administratorRepository.GetAdminInfoByUserName(administratorName);
            _administratorRepository.UpdateSiteId(adminInfo, channelInfo.Id);
        ...
        public void Delete(int siteId, int channelId)
        {
            ...
            {
                idList = ChannelManager.GetChannelIdList(channelInfo, ScopeType.Descendant, string.Empty, string.Empty, string.Empty);
            }
            ...
            if (channelInfo.ParentId == 0)
            {
                DataProvider.SiteRepository.Delete(channelInfo.Id);
            }
            else
            {
                ChannelManager.RemoveCacheBySiteId(channelInfo.SiteId);
        ...
        private static void QueryOrder(Query query, TaxisType taxisType)
        {
            ...
            else if (taxisType == TaxisType.OrderByAddDate || taxisType == TaxisType.OrderByLastEditDate)
            {
                query.OrderBy(Attr.AddDate);
            }
            else if (taxisType == TaxisType.OrderByAddDateDesc || taxisType == TaxisType.OrderByLastEditDateDesc)
            {
        ...
        private static void QueryWhereScope(Query query, int siteId, int channelId, bool isTotal, ScopeType scopeType)
        {
            if (isTotal)
            {
        ...
        private static void QueryWhereImage(Query query, bool? isImage)
        {
            if (isImage.HasValue)
            {
                if (isImage.Value)
