...
namespace SS.CMS.Core.Repositories
{
    public class SitePermissionsRepository : ISitePermissionsRepository
    {
        ...
        public SitePermissionsRepository(ISettingsManager settingsManager, IPermissionsInRolesRepository permissionsInRolesRepository)
        {
            _repository = new Repository<SitePermissionsInfo>(new Db(settingsManager.DatabaseType, settingsManager.DatabaseConnectionString));
        ...
        public int Insert(SitePermissionsInfo permissionsInfo)
        {
        ...
        {
            return _repository.Delete(Q.Where(Attr.RoleName, roleName)) == 1;
        }

        private void Delete(string roleName, int siteId)
        {
            _repository.Delete(Q.Where(Attr.RoleName, roleName).Where(Attr.SiteId, siteId));
        }

        private bool IsExists(string roleName, int siteId)
        {
            return _repository.Exists(Q.Where(Attr.RoleName, roleName).Where(Attr.SiteId, siteId));
        }
        ...
        public Dictionary<string, List<string>> GetChannelPermissionSortedList(IList<string> roles)
        {
            ...
            foreach (var roleName in roles)
            {
                ...
                foreach (var systemPermissionsInfo in systemPermissionsInfoList)
                {
                    ...
                    {
                        var key = Permissions.GetChannelPermissionDictKey(systemPermissionsInfo.SiteId, channelId);
        ...
        public void InsertRoleAndPermissions(string roleName, string creatorUserName, string description, List<string> generalPermissionList, List<SitePermissionsInfo> systemPermissionsInfoList)
        {
            if (generalPermissionList != null && generalPermissionList.Count > 0)
            {
                ...
                _permissionsInRolesRepository.Insert(permissionsInRolesInfo);
            }
            ...
            DataProvider.RoleRepository.InsertRole(new RoleInfo
            {
                RoleName = roleName,
...
// {
//     public class SitePermissionsDao
//     {
