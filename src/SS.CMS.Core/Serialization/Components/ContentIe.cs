...
namespace SS.CMS.Core.Serialization.Components
{
    internal class ContentIe
    {
        ...
        private void ImportContents(AtomEntryCollection entries, ChannelInfo channelInfo, int taxis, int importStart, int importCount, bool isCheckedBySettings, bool isChecked, int checkedLevel, bool isOverride, string adminName)
        {
            ...
            foreach (AtomEntry entry in entries)
            {
                try
                {
                    ...
                    {
                        topTaxis = taxis - 1;
                        taxis = channelInfo.ContentDao.GetMaxTaxis(channelInfo.Id, true) + 1;
                    }
                    ...
                    if (isInsert)
                    {
                        var contentId = channelInfo.ContentDao.InsertWithTaxis(_siteInfo, channelInfo, contentInfo, taxis);
        ...
        public bool ExportContents(SiteInfo siteInfo, int channelId, IList<int> contentIdList, bool isPeriods, string dateFrom, string dateTo, ETriState checkedState)
        {
            ...
            {
                contentIdList = channelInfo.ContentDao.GetContentIdList(channelId, isPeriods, dateFrom, dateTo, checkedState);
            }
