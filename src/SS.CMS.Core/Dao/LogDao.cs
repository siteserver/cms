...
{
    public class LogDao : IDatabaseDao
    {
        private readonly Repository<LogInfo> _repository;
        public LogDao(IDb db)
        {
            _repository = new Repository<LogInfo>(db);
        }
...
";//添加年统计
            ...
            using (var connection = _repository.Db.GetConnection())
            {
                using (var rdr = connection.ExecuteReader(sqlSelectTrackingDay))
                {
                    while (rdr.Read())
                    {
                        var accessNum = DatabaseUtils.GetInt(rdr, 0);
                        if (EStatictisXTypeUtils.Equals(xType, EStatictisXType.Day))
                        {
                            var year = DatabaseUtils.GetString(rdr, 1);
                            var month = DatabaseUtils.GetString(rdr, 2);
                            var day = DatabaseUtils.GetString(rdr, 3);
    ...
    WHERE {SqlUtils.GetDateDiffLessThanDays("AddDate", 30.ToString())} {builder}
            ...
            using (var connection = _repository.Db.GetConnection())
            {
                using (var rdr = connection.ExecuteReader(sqlSelectTrackingDay))
                {
                    ...
                    {
                        var accessNum = DatabaseUtils.GetInt(rdr, 0);
                        var userName = DatabaseUtils.GetString(rdr, 1);
