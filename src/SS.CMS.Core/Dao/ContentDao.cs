...
namespace SS.CMS.Core.Repositories
{
    public partial class ContentDao : IDatabaseDao
    {
        ...
        private ContentDao(IDb db, string tableName)
        {
            _repository = new Repository<ContentInfo>(db, tableName);
        }

        public string TableName => _repository.TableName;
        public List<TableColumn> TableColumns => _repository.TableColumns;

        public static List<TableColumn> GetDefaultTableColumns(IDb db)
        {
            return db.GetTableColumns<ContentInfo>();
        }

        private static readonly ConcurrentDictionary<string, ContentDao> ContentDaos = new ConcurrentDictionary<string, ContentDao>();

        public static ContentDao Instance(IDb db, string tableName)
        {
            ...
            repository = new ContentDao(db, tableName);
