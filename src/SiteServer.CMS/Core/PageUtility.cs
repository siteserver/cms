...
namespace SiteServer.CMS.Core
{
    public static class PageUtility
    {
        ...
        {
            if (site == null || string.IsNullOrEmpty(physicalPath)) return await site.GetWebUrlAsync();
        ...
        private static async Task<string> GetContentUrlByIdAsync(Site site, Content contentCurrent, bool isLocal)
        {
            ...
            {
                if (sourceId > 0 && await DataProvider.ChannelRepository.IsExistsAsync(sourceId))
                {
                ...
                {
                    var reference = await DataProvider.ContentRepository.GetAsync(site, channelId, referenceId);
        ...
        private static async Task<string> GetChannelUrlNotComputedAsync(Site site, int channelId, bool isLocal)
        {
            ...
            if (string.IsNullOrEmpty(linkUrl))
            {
                if (nodeInfo != null)
                {
                    ...
                    {
                        var channelUrl = await PathUtility.ChannelFilePathRules.ParseAsync(site, channelId);
        ...
        public static async Task<string> GetChannelUrlAsync(Site site, Channel channel, bool isLocal)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        var lastAddChannelInfo = await DataProvider.ChannelRepository.GetChannelByLastAddDateAsyncTask(site.Id,  channel.Id);
                    ...
                    {
                        var firstChannelInfo = await DataProvider.ChannelRepository.GetChannelByTaxisAsync(site.Id, channel.Id);
        ...
        public static List<Select<string>> GetLinkTypeSelects()
        {
