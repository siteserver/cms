...
namespace SiteServer.CMS.Repositories
{
    public partial class ChannelRepository
    {
        ...
        public async Task<Cascade<int>> GetCascadeCountAsync(Site site, IChannelSummary summary, int? count)
        {
            var cascade = new Cascade<int>
            {
                Value = summary.Id,
                Label = summary.ChannelName,
                Children = await GetCascadeChildrenCountAsync(site, summary.Id, count)
            };
            if (count.HasValue)
            {
                cascade["count"] = count;
        ...
        private async Task<List<Cascade<int>>> GetCascadeChildrenCountAsync(Site site, int parentId, int? count)
        {
            var list = new List<Cascade<int>>();
            ...
            foreach (var summary in summaries)
            {
                ...
                {
                    list.Add(await GetCascadeCountAsync(site, summary, count));
                }
            }
        ...
        public async Task<List<KeyValuePair<int, string>>> GetChannelsAsync(int siteId, IPermissions permissionsImpl, params string[] channelPermissions)
        {
