...
namespace SiteServer.CMS.Framework
{
    public static class WebConfigUtils
    {
        ...
        {
            PhysicalApplicationPath = settingsManager.ContentRootPath;

            IsProtectData = settingsManager.IsProtectData;
            DatabaseType = settingsManager.Database.DatabaseType;
            ConnectionString = settingsManager.Database.ConnectionString;
            RedisConnectionString = settingsManager.Redis.ConnectionString;
            AdminDirectory = settingsManager.AdminDirectory;
            HomeDirectory = settingsManager.HomeDirectory;
            SecretKey = settingsManager.SecurityKey;
            IsNightlyUpdate = settingsManager.IsNightlyUpdate;

            Database = new Database(DatabaseType, ConnectionString);
        ...
        public static void Load(string physicalApplicationPath, string webConfigPath)
        {
            ...
            try
            {
                ...
                if (appSettings != null)
                {
                    ...
                    if (isProtectData)
                    {
                        databaseType = TranslateUtils.DecryptStringBySecretKey(databaseType, SecretKey);
                        connectionString = TranslateUtils.DecryptStringBySecretKey(connectionString, SecretKey);
                    }
                }
            }
            catch
            ...
            DatabaseType = TranslateUtils.ToEnum(databaseType, DatabaseType.MySql);
                    ...
                    if (setting.Name == "add")
                    {
                        ...
                        if (attrKey != null)
                        {
                            ...
                            else if (StringUtils.EqualsIgnoreCase(attrKey.Value, nameof(DatabaseType)))
                            {
                                ...
                                if (attrValue != null)
                                {
                                    attrValue.Value = databaseType.GetValue();
                                    if (isProtectData)
                                    {
