    ...
    public partial class LayerAddMultipleController : ControllerBase
    {
        ...
        public async Task<ActionResult<BoolResult>> Submit([FromBody] SubmitRequest request)
        {
            foreach (var style in request.Styles)
            {
                ...
                var tableStyle = await _databaseManager.IsAttributeNameExistsAsync(request.TableName, style.AttributeName) ? await _tableStyleRepository.GetTableStyleAsync(request.TableName, style.AttributeName, request.RelatedIdentities) : new Models.TableStyle();
