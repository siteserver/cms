    ...
    public static class BackgroundInputTypeParser
    {
        ...
        public static string ParseText(IDictionary<string, object> attributes, SiteInfo siteInfo, int channelId, TableStyleInfo styleInfo, StringBuilder extraBuilder)
        {
            ...
            if (styleInfo.FormatString)
            {
                ...
                var formatValues = TranslateUtils.Get<string>(attributes, ContentAttribute.GetFormatStringAttributeName(styleInfo.AttributeName), string.Empty);
...
<div id=""titleTips"" class=""inputTips""></div>");
            ...
            var value = StringUtils.HtmlDecode(TranslateUtils.Get<string>(attributes, styleInfo.AttributeName, string.Empty));
        ...
        {
            var value = TranslateUtils.Get<string>(attributes, attributeName, string.Empty);
...
$(function(){{
  ...
  $('#{attributeName}').show();
        ...
        private static string ParseSelectOne(IDictionary<string, object> attributes, TableStyleInfo styleInfo, StringBuilder extraBuilder)
        {
            ...
            var selectedValue = TranslateUtils.Get<string>(attributes, styleInfo.AttributeName, string.Empty);
        ...
        private static string ParseSelectMultiple(IDictionary<string, object> attributes, TableStyleInfo styleInfo, StringBuilder extraBuilder)
        {
            ...
            var selectedValues = TranslateUtils.StringCollectionToStringList(TranslateUtils.Get<string>(attributes, styleInfo.AttributeName, string.Empty));
        ...
        private static string ParseSelectCascading(IDictionary<string, object> attributes, SiteInfo siteInfo, TableStyleInfo styleInfo, StringBuilder extraBuilder)
        {
            ...
            var fieldInfo = DataProvider.RelatedFieldDao.GetRelatedFieldInfo(styleInfo.RelatedFieldId);
            if (fieldInfo == null) return string.Empty;

            var list = DataProvider.RelatedFieldItemDao.GetRelatedFieldItemInfoList(styleInfo.RelatedFieldId, 0);
...
<span id=""c_{attributeName}_1"">
    ...
    <select name=""{attributeName}"" id=""{attributeName}_1"" class=""select"" onchange=""getRelatedField_{fieldInfo.Id}(2);"">
        <option value="""">请选择</option>");
            ...
            var values = TranslateUtils.Get<string>(attributes, attributeName, string.Empty);
...
");
            ...
            var extendValues = TranslateUtils.Get<string>(attributes, extendAttributeName, string.Empty);
            ...
            return $@"<input id=""{attributeName}"" name=""{attributeName}"" type=""text"" class=""form-control"" value=""{TranslateUtils.Get<string>(attributes, attributeName, string.Empty)}"" />";
        }
...
");
        ...
        private static string ParseCustomize(IDictionary<string, object> attributes, TableStyleInfo styleInfo, StringBuilder extraBuilder)
        {
            ...
            var value = TranslateUtils.Get<string>(attributes, styleInfo.AttributeName, string.Empty);
