...
class IndexPage extends React.Component {
    ...
    loadContents(publishmentSystemId, nodeId, page) {
        ...
        const dateFrom = '';
        const dateTo = '';
        client.writing.getContents(publishmentSystemId, nodeId, searchType, keyword, dateFrom, dateTo, page, (err, res) => {
    ...
    render() {
        ...
        if (this.state.nodes.length > 0) {
            ...
            nodesEl = (<Select placeholder="全部栏目" clearable={true} options={options} value={this.state.node ? this.state.node.nodeId : 0} onChange={(item) => {
                if (item) {
                    this.state.node = {
                        nodeId: item.value,
                        nodeName: item.label
            ...
            if (!isChecked) {
                ...
                deleteEl = <a href="javascript:;" className="lnk" onClick={() => {
                    utils.Swal.confirm('此操作将删除稿件，确认吗？', (isConfirm) => {
                        if (isConfirm) {
                            client.writing.deleteContent(publishmentSystemId, nodeId, id, (err, res) => {
