    ...
    public static class BackupJob
    {
        ...
        public static async Task Execute(IJobContext context)
        {
            ...
            foreach (var tableName in tableNames)
            {
                ...
                var identityColumnName = DatabaseUtils.AddIdentityColumnIdIfNotExists(tableName, tableInfo.Columns);
                ...
                if (tableInfo.TotalCount > 0)
                {
                    ...
                    if (tableInfo.TotalCount > CliUtils.PageSize)
                    {
                        ...
                        using (var progress = new ProgressBar())
                        {
                            for (; current <= pageCount; current++)
                            {
                                ...
                                var rows = DatabaseUtils.GetPageObjects(tableName, identityColumnName, offset, limit);
                    ...
                    else
                    {
                        var fileName = $"{current}.json";
                        tableInfo.RowFiles.Add(fileName);
                        var rows = DatabaseUtils.GetObjects(tableName);
