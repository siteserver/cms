...
namespace SiteServer.CMS.StlParser.Cache
{
    public class SeoMeta
    {
        ...
        {
            var cacheKey = StlCacheUtils.GetCacheKeyByGuid(guid, nameof(SeoMeta), nameof(GetSeoMetaIdByNodeId), nodeId.ToString(), isChannel.ToString());
        ...
        {
            var cacheKey = StlCacheUtils.GetCacheKeyByGuid(guid, nameof(SeoMeta), nameof(GetDefaultSeoMetaId), publishmentSystemId.ToString());
            var retval = StlCacheUtils.GetIntCache(cacheKey);
            if (retval != -1) return retval;
        ...
        {
            var cacheKey = StlCacheUtils.GetCacheKeyByGuid(guid, nameof(SeoMeta), nameof(GetSeoMetaInfo), seoMetaId.ToString());
