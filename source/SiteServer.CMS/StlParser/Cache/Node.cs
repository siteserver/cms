...
namespace SiteServer.CMS.StlParser.Cache
{
    public class Node
    {
        ...
        {
            var cacheKey = StlCacheUtils.GetCacheKeyByGuid(guid, nameof(Node), nameof(GetPublishmentSystemId), nodeId.ToString());
        ...
        {
            var cacheKey = StlCacheUtils.GetCacheKeyByGuid(guid, nameof(Node), nameof(GetNodeIdListByScopeType), nodeId.ToString(), childrenCount.ToString(), EScopeTypeUtils.GetValue(scope), group, groupNot);
        ...
        {
            var cacheKey = StlCacheUtils.GetCacheKeyByGuid(guid, nameof(Node), nameof(GetNodeIdByNodeIndexName), publishmentSystemId.ToString(), channelIndex);
            var retval = StlCacheUtils.GetIntCache(cacheKey);
            if (retval != -1) return retval;
        ...
        {
            var cacheKey = StlCacheUtils.GetCacheKeyByGuid(guid, nameof(Node), nameof(GetNodeIdByParentIdAndTaxis), parentId.ToString(), taxis.ToString(), isNextChannel.ToString());
            var retval = StlCacheUtils.GetIntCache(cacheKey);
            if (retval != -1) return retval;
        ...
        {
            var cacheKey = StlCacheUtils.GetCacheKeyByGuid(guid, nameof(Node), nameof(GetNodeIdByChannelIdOrChannelIndexOrChannelName), publishmentSystemId.ToString(), channelId.ToString(), channelIndex, channelName);
            var retval = StlCacheUtils.GetIntCache(cacheKey);
            if (retval != -1) return retval;
        ...
        {
            var cacheKey = StlCacheUtils.GetCacheKeyByGuid(guid, nameof(Node), nameof(GetWhereString), publishmentSystemId.ToString(), groupContent, groupContentNot, isImageExists.ToString(), isImage.ToString(), where);
        ...
        {
            var cacheKey = StlCacheUtils.GetCacheKeyByGuid(guid, nameof(Node), nameof(GetNodeIdList), channelId.ToString(), totalNum.ToString(), orderByString, whereString, EScopeTypeUtils.GetValue(scopeType), groupChannel, groupChannelNot);
            var retval = StlCacheUtils.GetCache<List<int>>(cacheKey);
            if (retval != null) return retval;
