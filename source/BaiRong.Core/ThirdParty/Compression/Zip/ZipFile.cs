...
namespace ICSharpCode.SharpZipLib.Zip 
{
	...
	public class ZipFile : IEnumerable
	{
		...
		void ReadEntries()
		{
			...
			var locatedCentralDirOffset = LocateBlockWithSignature(ZipConstants.ENDSIG, baseStream.Length, ZipConstants.ENDHDR, 0xffff);
...
*/
			...
			for (var i = 0; i < entriesForThisDisk; i++) {
		...
		public bool TestArchive(bool testData)
		{
			...
			try {
				for (var i = 0; i < Size; ++i) {
					var offset = TestLocalHeader(this[i], true, true);
					if (testData) {
						var entryStream = GetInputStream(this[i]);
		...
		long TestLocalHeader(ZipEntry entry, bool fullTest, bool extractTest)
		{
			lock(baseStream) 
			{
				...
				if (extractTest == true && shortValue > ZipConstants.VERSION_MADE_BY) {
					throw new ZipException($"Version required to extract this entry not supported ({shortValue})");
				}

				var localFlags = (short)ReadLeShort();				  // general purpose bit flags.
				if (extractTest == true) {
		...
		void ReadFully(Stream s, byte[] outBuf)
		{
			...
			while (len > 0) {
				var count = s.Read(outBuf, off, len);
				if (count <= 0) {
