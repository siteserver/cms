    ...
    public class FastZip
    {
        ...
        void ProcessDirectory(object sender, DirectoryEventArgs e)
        {
            if (!e.HasMatchingFiles && createEmptyDirectories)
            {
                ...
                {
                    var cleanedName = nameTransform.TransformDirectory(e.Name);
                    var entry = new ZipEntry(cleanedName);
        ...
        void ProcessFile(object sender, ScanEventArgs e)
        {
            ...
            var cleanedName = nameTransform.TransformFile(e.Name);
            var entry = new ZipEntry(cleanedName);
            outputStream.PutNextEntry(entry);
        ...
        void ExtractFileEntry(ZipEntry entry, string targetName)
        {
            ...
            if (proceed)
            {
                ...
                var streamWriter = File.Create(targetName);
        ...
        {
            var doExtraction = NameIsValid(entry.Name);
            ...
            if (doExtraction)
            {
                ...
                {
                    var workName = Path.GetPathRoot(entry.Name);
            ...
            if (doExtraction && !Directory.Exists(dirName))
            {
                if (!entry.IsDirectory || CreateEmptyDirectories)
                {
                    try
                    {
                        if (!dirName.EndsWith("\\"))
                            dirName = dirName + "\\";
                        DirectoryUtils.CreateDirectoryIfNotExists(dirName);
                    }
                    catch
                    {
                        doExtraction = false;
