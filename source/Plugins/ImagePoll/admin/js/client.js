...
Client.parse = function(responseText) {
    return responseText ? JSON.parse(responseText) : {};
};

Client.getQueryString = function() {
    var i, len, params = '',
...
Client.errorCode = function(status) {
    switch (status) {
        case 400:
            return 'Bad Request';
        case 401:
    ...
    _request: function(method, path, data, cb) {
        ...
        if (cb) {
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4) {
                    if (xhr.status < 400) {
                        cb(null, Client.parse(xhr.responseText), xhr.status);
                    } else {
                        var err = Client.parse(xhr.responseText);
                        cb({
                            status: xhr.status,
                            message: err.message || Client.errorCode(xhr.status)
                        }, null, xhr.status);
                    }
                }
            };
