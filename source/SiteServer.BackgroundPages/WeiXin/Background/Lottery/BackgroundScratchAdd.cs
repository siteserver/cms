...
{
	public class BackgroundScratchAdd : BackgroundBasePageWX
	{
        ...
        public static string GetRedirectUrl(int publishmentSystemID, int lotteryID)
        {
            return PageUtils.GetWXUrl(
                $"background_scratchAdd.aspx?publishmentSystemID={publishmentSystemID}&lotteryID={lotteryID}");
        }
		...
		public void Page_Load(object sender, EventArgs e)
        {
			...
			if (!IsPostBack)
            {
                var pageTitle = lotteryID > 0 ? "编辑刮刮卡" : "添加刮刮卡";
                BreadCrumb(AppManager.WeiXin.LeftMenu.ID_Function, AppManager.WeiXin.LeftMenu.Function.ID_Scratch, pageTitle, AppManager.WeiXin.Permission.WebSite.Scratch);
...
itemController.itemCount = {awardInfoList.Count};itemController.items = [{itemBuilder.ToString()}];";
                ...
                btnReturn.Attributes.Add("onclick",
                    $@"location.href=""{BackgroundLottery.GetRedirectUrl(PublishmentSystemID, ELotteryType.Scratch)}"";return false");
			}
		}
		...
		public override void Submit_OnClick(object sender, EventArgs e)
		{
			if (Page.IsPostBack && Page.IsValid)
			{
                ...
                else if (selectedStep == 5)
                {
                    ...
                    lotteryInfo.KeywordID = DataProviderWX.KeywordDAO.GetKeywordID(PublishmentSystemID, lotteryID > 0, tbKeywords.Text, EKeywordType.Scratch, lotteryInfo.KeywordID);
                    ...
                    try
                    {
                        if (lotteryID > 0)
                        {
                            ...
                            LogUtils.AddLog(BaiRongDataProvider.AdministratorDao.UserName, "修改刮刮卡",
                                $"刮刮卡:{tbTitle.Text}");
                        ...
                        else
                        {
                            ...
                            LogUtils.AddLog(BaiRongDataProvider.AdministratorDao.UserName, "添加刮刮卡",
                                $"刮刮卡:{tbTitle.Text}");
                            SuccessMessage("添加刮刮卡成功！");
                        }

                        var redirectUrl = BackgroundLottery.GetRedirectUrl(PublishmentSystemID, ELotteryType.Scratch);
