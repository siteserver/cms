    ...
    public class BackgroundFlapAdd : BackgroundBasePageWX
	{
        ...
        public static string GetRedirectUrl(int publishmentSystemID, int lotteryID)
        {
            return PageUtils.GetWXUrl(
                $"background_flapAdd.aspx?publishmentSystemID={publishmentSystemID}&lotteryID={lotteryID}");
        }
		...
		public void Page_Load(object sender, EventArgs e)
        {
			...
			if (!IsPostBack)
            {
                var pageTitle = lotteryID > 0 ? "编辑大翻牌" : "添加大翻牌";
                BreadCrumb(AppManager.WeiXin.LeftMenu.ID_Function, AppManager.WeiXin.LeftMenu.Function.ID_Flap, pageTitle, AppManager.WeiXin.Permission.WebSite.Flap);
...
itemController.itemCount = {awardInfoList.Count};itemController.items = [{itemBuilder.ToString()}];";
                ...
                btnReturn.Attributes.Add("onclick",
                    $@"location.href=""{BackgroundLottery.GetRedirectUrl(PublishmentSystemID, ELotteryType.Flap)}"";return false");
			}
		}
		...
		public override void Submit_OnClick(object sender, EventArgs e)
		{
			if (Page.IsPostBack && Page.IsValid)
			{
                ...
                else if (selectedStep == 5)
                {
                    ...
                    lotteryInfo.KeywordID = DataProviderWX.KeywordDAO.GetKeywordID(PublishmentSystemID, lotteryID > 0, tbKeywords.Text, EKeywordType.Flap, lotteryInfo.KeywordID);
                    ...
                    try
                    {
                        if (lotteryID > 0)
                        {
                            ...
                            LogUtils.AddLog(BaiRongDataProvider.AdministratorDao.UserName, "修改大翻牌",
                                $"大翻牌:{tbTitle.Text}");
                        ...
                        else
                        {
                            ...
                            LogUtils.AddLog(BaiRongDataProvider.AdministratorDao.UserName, "添加大翻牌",
                                $"大翻牌:{tbTitle.Text}");
                            SuccessMessage("添加大翻牌成功！");
                        }

                        var redirectUrl = BackgroundLottery.GetRedirectUrl(PublishmentSystemID, ELotteryType.Flap);
