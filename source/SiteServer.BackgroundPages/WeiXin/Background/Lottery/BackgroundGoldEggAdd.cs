...
{
    public class BackgroundGoldEggAdd : BackgroundBasePageWX
	{
        ...
        public static string GetRedirectUrl(int publishmentSystemID, int lotteryID)
        {
            return PageUtils.GetWXUrl(
                $"background_goldEggAdd.aspx?publishmentSystemID={publishmentSystemID}&lotteryID={lotteryID}");
        }
		...
		public void Page_Load(object sender, EventArgs e)
        {
			...
			if (!IsPostBack)
            {
                var pageTitle = lotteryID > 0 ? "编辑砸金蛋" : "添加砸金蛋";
                BreadCrumb(AppManager.WeiXin.LeftMenu.ID_Function, AppManager.WeiXin.LeftMenu.Function.ID_GoldEgg, pageTitle, AppManager.WeiXin.Permission.WebSite.GoldEgg);
...
itemController.itemCount = {awardInfoList.Count};itemController.items = [{itemBuilder.ToString()}];";
                ...
                btnReturn.Attributes.Add("onclick",
                    $@"location.href=""{BackgroundLottery.GetRedirectUrl(PublishmentSystemID, ELotteryType.GoldEgg)}"";return false");
			}
		}
