...
{
    public class BackgroundBigWheelAdd : BackgroundBasePageWX
    {
        ...
        public static string GetRedirectUrl(int publishmentSystemID, int lotteryID)
        {
            return PageUtils.GetWXUrl(
                $"background_bigWheelAdd.aspx?publishmentSystemID={publishmentSystemID}&lotteryID={lotteryID}");
        }
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                var pageTitle = lotteryID > 0 ? "编辑大转盘" : "添加大转盘";
                BreadCrumb(AppManager.WeiXin.LeftMenu.ID_Function, AppManager.WeiXin.LeftMenu.Function.ID_BigWheel, pageTitle, AppManager.WeiXin.Permission.WebSite.BigWheel);
...
itemController.itemCount = {awardInfoList.Count};itemController.items = [{itemBuilder.ToString()}];";
                ...
                btnReturn.Attributes.Add("onclick",
                    $@"location.href=""{BackgroundLottery.GetRedirectUrl(PublishmentSystemID, ELotteryType.BigWheel)}"";return false");
            }
        }
        ...
        public override void Submit_OnClick(object sender, EventArgs e)
        {
            if (Page.IsPostBack && Page.IsValid)
            {
                ...
                else if (selectedStep == 5)
                {
                    ...
                    lotteryInfo.KeywordID = DataProviderWX.KeywordDAO.GetKeywordID(PublishmentSystemID, lotteryID > 0, tbKeywords.Text, EKeywordType.BigWheel, lotteryInfo.KeywordID);
                    ...
                    try
                    {
                        if (lotteryID > 0)
                        {
                            ...
                            LogUtils.AddLog(BaiRongDataProvider.AdministratorDao.UserName, "修改大转盘",
                                $"大转盘:{tbTitle.Text}");
                        ...
                        else
                        {
                            ...
                            LogUtils.AddLog(BaiRongDataProvider.AdministratorDao.UserName, "添加大转盘",
                                $"大转盘:{tbTitle.Text}");
                            SuccessMessage("添加大转盘成功！");
                        }

                        var redirectUrl = BackgroundLottery.GetRedirectUrl(PublishmentSystemID, ELotteryType.BigWheel);
