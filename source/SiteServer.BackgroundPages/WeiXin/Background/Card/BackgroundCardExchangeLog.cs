...
namespace SiteServer.WeiXin.BackgroundPages
{
    public class BackgroundCardExchangeLog : BackgroundBasePageWX
    {
        ...
        {
            return PageUtils.GetWXUrl("background_cardExchangeLog.aspx?PublishmentSystemID=" + publishmentSystemID + "&cardSN=" + cardSN + "&userName=" + userName + "&mobile=" + mobile);
        }
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                BreadCrumb(AppManager.WeiXin.LeftMenu.ID_Function, AppManager.WeiXin.LeftMenu.Function.ID_Card, "会员积分兑换管理", AppManager.WeiXin.Permission.WebSite.Card);
                ...
                var urlDelete = PageUtils.AddQueryString(BackgroundCardConsumeLog.GetRedirectUrl(PublishmentSystemID, tbCardSN.Text, tbUserName.Text, tbMobile.Text), "Delete", "True");
                btnDelete.Attributes.Add("onclick", JsUtils.GetRedirectStringWithCheckBoxValueAndAlert(urlDelete, "IDCollection", "IDCollection", "请选择需要删除的兑换记录", "此操作将删除所选兑换记录，确认吗？"));
            ...
            get
            {
                if (string.IsNullOrEmpty(_pageUrl))
                {
                    _pageUrl = PageUtils.GetWXUrl(
                        $"background_cardExchangeLog.aspx?PublishmentSystemID={PublishmentSystemID}&cardID={ddlCard.SelectedValue}&cardSN={tbCardSN.Text}&userName={tbUserName.Text}&mobile={tbMobile.Text}");
                }
                return _pageUrl;
            }
        }
