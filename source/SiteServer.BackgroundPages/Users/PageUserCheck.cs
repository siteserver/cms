...
namespace SiteServer.BackgroundPages.Users
{
    public class PageUserCheck : BasePage
    {
        ...
        {
            return PageUtils.GetUsersUrl(nameof(PageUserCheck), null);
        }
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                BreadCrumbUsers("审核新用户", AppManager.Permissions.Users.UserManagement);

                spContents.DataBind();

                Check.Attributes.Add("onclick",
                    PageUtils.GetRedirectStringWithCheckBoxValueAndAlert(
                        PageUtils.GetUsersUrl(nameof(PageUserCheck), new NameValueCollection
                        {
                            {"Check", "True"}
