...
namespace SiteServer.BackgroundPages.Users
{
    public class PageArea : BasePage
    {
        ...
        public static string GetRedirectUrl(int currentAreaId)
        {
            if (currentAreaId > 0)
            {
                return PageUtils.GetUsersUrl(nameof(PageArea), new NameValueCollection
                {
                    {"CurrentAreaID", currentAreaId.ToString()}
                });
            }
            return PageUtils.GetUsersUrl(nameof(PageArea), null);
        }
        ...
        public void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                BreadCrumbUsers("所在区域管理", AppManager.Permissions.Users.AdminConfiguration);
                ...
                var urlDelete = PageUtils.GetUsersUrl(nameof(PageArea), new NameValueCollection
                {
                    {"Delete", "True"}
                });
        ...
        public static string GetAreaRowHtml(AreaInfo areaInfo, EAreaLoadingType loadingType, NameValueCollection additional)
        {
            ...
            if (loadingType == EAreaLoadingType.Management)
            {
                ...
                var urlUp = PageUtils.GetUsersUrl(nameof(PageArea), new NameValueCollection
                {
                    {"Subtract", "True"},
                ...
                var urlDown = PageUtils.GetUsersUrl(nameof(PageArea), new NameValueCollection
                {
                    {"Add", "True"},
                    {"AreaID", areaInfo.AreaId.ToString()}
                });
