...
namespace SiteServer.BackgroundPages.Users
{
    public class ModalUserGroupAdd : BasePage
    {
        ...
        {
            return PageUtils.GetOpenWindowString("修改用户组", PageUtils.GetUsersUrl(nameof(ModalUserGroupAdd), new NameValueCollection
            {
        ...
        {
            return PageUtils.GetOpenWindowString("添加用户组", PageUtils.GetUsersUrl(nameof(ModalUserGroupAdd), new NameValueCollection
            {
                {"GroupID", groupId.ToString()},
                {"IsAdd", true.ToString()}
            }), 620, 550);
