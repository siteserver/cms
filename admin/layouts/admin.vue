                  ...
                  <li v-for="parentMenu in topMenu.children" :key="parentMenu.id" :class="{'has-submenu': parentMenu.children}">
                    ...
                    <ul v-if="parentMenu.hasChildren" class="submenu">
                      <li v-for="childMenu in parentMenu.children" :key="childMenu.id">
                        ...
                        <nuxt-link :to="{ path: getHref(childMenu), query: $route.query }">
                          {{ childMenu.text }}
                        </nuxt-link>
                      </li>
                    </ul>
            ...
            <div class="menu-extras">
              <ul class="navigation-menu nav navbar-nav navbar-right float-right">
                <li v-if="newVersion || updatePackages.length > 0" class="has-submenu dropdown">
                  <a href="javascript:;">
                    <i class="fa fa-flash text-warning" />
          ...
          <div id="sidebar-menu">
            <ul>
              <li v-for="siteMenu in siteMenus" :key="siteMenu.id" class="has_sub">
                ...
                >
                  <i :class="(siteMenu.iconClass ? siteMenu.iconClass : 'fa fa-star')" />
                  {{ siteMenu.text }}
              ...
              <li v-for="pluginMenu in pluginMenus" :key="pluginMenu.id" class="has_sub">
                ...
                >
                  <i :class="(pluginMenu.iconClass ? pluginMenu.iconClass : 'fa fa-star')" />
                  {{ pluginMenu.text }}
              ...
              <template v-for="topMenu in topMenus">
                <template v-if="!isDesktop && isMobileMenu">
                  ...
                  <li v-for="parentMenu in topMenu.children" :key="parentMenu.id" class="has_sub">
                    ...
                    >
                      <i :class="(parentMenu.iconClass ? parentMenu.iconClass : 'fa fa-star')" />
                      {{ parentMenu.text }}
...
export default {
  ...
  methods: {
    async apiValidate() {
      ...
      const data = await this.$api.root.postAt('actions/validate', {
        siteId: this.siteId
      })
      ...
      // isLockedOut: false
      // productVersion: null
      // redirectToCreateSite: false
      // redirectToErrorSite: false
      // siteId: 0
      if (!data.isInstalled) {
        this.$router.push(this.$constants.url.install)
      }
    },
    async apiGet() {
      this.$store.commit('loading', true)

      const data = await this.$api.root.get({
    ...
    async create() {
      ...
      const data = await this.$api.root.post()
    ...
    async downloadSsCms(releaseInfo) {
      ...
      const data = await this.$api.root.postAt('/actions/download', {
        packageId: 'SS.CMS',
    ...
    getHref(menu) {
      const pageUrl = menu.href && menu.target !== '_layer' ? menu.href : 'javascript:;'
      return pageUrl.replace('/page', '/').replace('.aspx', '').replace('.cshtml', '')
    },
    ...
    openPageCreateStatus() {
      this.$modal.openLayer({
        title: '生成进度查看',
        url: `cms/createStatus?siteId=${this.siteId}`,
        full: true
